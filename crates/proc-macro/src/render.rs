...
pub fn render_vixen_parser(idl: &RootNode) -> TokenStream {
    ...
    let instruction_parser = render_instruction_parser(&idl.program.name, &idl.program.instructions);

    quote! {
        mod #program_mod_ident {
            use borsh::{BorshDeserialize, BorshSerialize};
            use yellowstone_vixen_core::{Parser, ParseResult, ParseError, Prefilter, PrefilterBuilder, Pubkey, KeyBytes, AccountUpdate};
            use yellowstone_vixen_core::instruction;

            const ID: yellowstone_vixen_core::Pubkey = #program_pubkey;
    ...
    quote! {
        {
            ...
            yellowstone_vixen_core::KeyBytes::<32>(BYTES)
        }
    }
}
...
fn render_instructions(instructions: &[codama_nodes::InstructionNode]) -> TokenStream {
    let sections = instructions.iter().map(|instruction| {
        ...
        let accounts_fields = instruction.accounts.iter().map(|account| {
            ...
            quote! {
                pub #field_ident: yellowstone_vixen_core::Pubkey
            }
        });
                ...
                if let NestedTypeNode::Value(struct_node) = &account.data {
                    match struct_node
                        ...
                        .and_then(|field| {
                            ...
                            field.default_value.as_ref().and_then(|value| {
                                match value {
...
fn render_instruction_parser(
    ...
    let instruction_matches = instructions.iter().filter_map(|instruction| {
        ...
        let instr_accounts_fields = instruction
            .accounts
            .iter()
            .enumerate()
            .map(|(idx, account)| {
                ...
                quote! {
                    #field: *accounts.get(#idx).ok_or(ParseError::from(#error_msg))?
                }
            });
...
fn quoted_type_node(type_node: &codama_nodes::TypeNode) -> TokenStream {
    ...
    match type_node {
        ...
        PublicKey(_) => quote! { yellowstone_vixen_core::Pubkey },
...
mod tests {
    ...
    fn test_generate_vixen_parser() {
            ...
            accounts: vec![AccountNode::new(
                ...
                StructTypeNode::new(vec![
                    ...
                    StructFieldTypeNode::new(
                        "level",
                        OptionTypeNode::new(NumberTypeNode::le(U64)),
                    ),
        ...
        let expected_tokens: proc_macro2::TokenStream = quote::quote! {
            mod test {
                use borsh::{BorshDeserialize, BorshSerialize};
                use yellowstone_vixen_core::prelude::*;

                const ID: yellowstone_vixen_core::Pubkey = {
                ...
                pub struct MyAccount {
                    ...
                    pub teammate: yellowstone_vixen_core::Pubkey,
                }
